...
class xrowVideoGetLanguageOperator
{
    ...
    function modify( $tpl, $operatorName, $operatorParameters, &$rootNamespace, &$currentNamespace, &$operatorValue, &$namedParameters )
    {
        switch ( $operatorName )
        {
            case 'get_language':
            {
                if ( isset( $namedParameters['alpha3country'] ) )
                {
                    $country = explode("-", $namedParameters['alpha3country']);
                    $country = $country[0];
                    $country_item = $this::getLanguage($country);
                    $operatorValue = strtolower($country_item["alpha2"]);
                }
            }break;
        }
    }
